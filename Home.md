...
###Network
A PJON network is made by a collection of up to 255 devices transmitting and receiving on the same medium. Communication between devices occurs through packets, and communication is based on democracy: every device has the right to transmit for up to `(1000 / devices number) milliseconds / second`.   
